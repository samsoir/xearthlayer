...
- **Memory grew for as long as you flew, until the system killed the process** ([#227](https://github.com/samsoir/xearthlayer/issues/227)): A twelve-hour flight ended in an out-of-memory kill at 64 GB against a configured 4 GB cache, with no application error and nothing in the log to separate the candidate causes.
  ...
  **Nothing was leaking.** Bytes actually in use stayed flat at 572–725 MB across an eighteen-fold increase in work; everything was being freed. What grew was the C allocator's *retention* of freed memory, which is why nothing in the code ever looked wrong.

  glibc gives threads separate heap arenas to avoid lock contention, and returns memory to the operating system only by trimming the **top** of one. Space stranded below a still-live allocation stays committed for the life of the process, so an arena grows to cover the worst burst it ever saw and never gives that space back. The total is the sum of every arena's personal high-water mark — and with a ceiling of eight arenas per core, each new burst could recruit *fresh* arenas that had never seen a peak. Rare traffic peaks were being converted into permanent floor, one arena at a time.
