...
- **Preflight check framework** ([#265](https://github.com/samsoir/xearthlayer/issues/265)): Startup prerequisites are now declared and registered rather than accumulated. Seven checks lived inline at the top of the `run` command, unnamed, in an order fixed by the accident of when each was added. That order was load-bearing and nothing recorded it: the configuration upgrade warning has to follow the first-run check because it reads the config file, and the packages check has to follow configuration load because it consumes a config value. Neither constraint was stated anywhere and no test protected them.

  They now live in an ordered registry that runs before command dispatch rather than inside `run`. Placement was the point. `run` and the setup wizard each answered "does this installation exist" independently from the same state, so a check living in `run` alone left `xearthlayer setup` treating an existing user as new and offering to overwrite a working configuration.

  Each check declares whether it applies to the command being run, inspects without side effects, and acts only when asked to. Because inspection is pure, the whole registry can be run in report mode, which is what a diagnostics prerequisites section and any dry run will use. Checks never call one another: they read from and contribute to a bootstrap context that accumulates the validated inputs XEarthLayer launches from, so a misordered registry produces an error naming the value it lacked rather than a panic. The context carries inputs, not constructed services, so the existing builders keep ownership of startup. The `run` command no longer re-resolves anything and lost 197 lines.

  Two prerequisites that should have existed and did not are now included. **macFUSE presence** on macOS, where a missing or unapproved kernel extension previously surfaced as an opaque mount failure that said nothing about the cause; it now names the problem and points at the approval steps in `docs/macos.md`. And a **running instance guard**, which nothing provided before, so a second instance could mount the same paths and share a cache directory with the first. A lock whose owner is gone is cleared and the run proceeds, because refusing to start until a user deletes a file would turn any crash into an installation that looks broken with no obvious cause.

  Introducing the framework changed no behaviour. The test suite cannot see terminal output, so that claim was checked by building a binary from the base commit and diffing five startup scenarios against it: fresh installation, no packages, missing Custom Scenery, invalid airport, and unresolvable scenery. All five matched byte for byte, including the exit code of zero a fresh installation returns. The comparison caught one regression that 66 new tests did not, where the version banner had stopped being written to the log whenever a check failed, which is exactly when a support log needs to record which build produced the failure.
...
- **Committed-memory and allocator telemetry** ([#227](https://github.com/samsoir/xearthlayer/issues/227)): The `Memory sample` line gains `anon_mb` and four glibc accounting fields. `anon_mb + swap_mb` is the process's committed footprint, what the OOM killer scores, and should be quoted in place of `vm_mb`, which also counts address space that is mapped but never touched. Thread stacks measured 2.21 MB each against a pool cycling between 37 and 586 threads, so `vm_mb` carries a sawtooth of roughly a gigabyte that is not memory at all; two people reading this trace mistook that artifact for a finding before it was identified. `heap_mb`, `mmap_mb`, `inuse_mb` and `heap_free_mb` separate arena retention from genuine retention: `heap_free_mb` rising is the allocator holding free space, `inuse_mb` rising is the process holding objects, and all four flat while `anon_mb` climbs means the growth is outside malloc entirely. A `malloc_trim` line at shutdown reports how much of the footprint was releasable.
...
- **Memory grew for as long as you flew, until the system killed the process** ([#227](https://github.com/samsoir/xearthlayer/issues/227)): A twelve-hour flight ended in an out-of-memory kill at 64 GB against a configured 4 GB cache, with no application error and nothing in the log to separate the candidate causes.

  **Nothing was leaking.** Bytes actually in use stayed flat at 572 to 725 MB across an eighteen-fold increase in work; everything was being freed. What grew was the C allocator's *retention* of freed memory, which is why nothing in the code ever looked wrong.

  glibc gives threads separate heap arenas to avoid lock contention, and returns memory to the operating system only by trimming the **top** of one. Space stranded below a still-live allocation stays committed for the life of the process, so an arena grows to cover the worst burst it ever saw and never gives that space back. The total is the sum of every arena's personal high-water mark. With a ceiling of eight arenas per core, each new burst could recruit *fresh* arenas that had never seen a peak. Rare traffic peaks were being converted into permanent floor, one arena at a time.

  Three mechanisms, fixed together. A generated texture is 11.17 MiB, and glibc's threshold for serving an allocation that size by `mmap`, which does return memory on free, adapts *upward* past that size after a single tile, after which every arena serving a tile keeps the space; the threshold is now pinned, at a measured 0.16 ms per tile. The thread pool carrying encoding and disk I/O accepted the runtime's 512-thread default and reached 581 threads, each excursion ratcheting the high-water mark; it is now sized from the work that actually queues on it. And the arena count itself is now capped, so once each arena has seen its worst burst there are no fresh ones left to recruit and the total converges.

  Measured across matched eleven-hour flights, before and after: committed memory **11,580 → 9,687 MB**, allocator arenas **6,091 → 4,038 MB**, swap **1,063 → 533 MB**, and growth steps after initial sizing **18 → 1**. Throughput was unchanged at 3.03 against 3.06 textures per second.

  Both allocator settings defer to an explicit `MALLOC_MMAP_THRESHOLD_`, `MALLOC_ARENA_MAX`, or `GLIBC_TUNABLES` entry, so anyone tuning the allocator themselves keeps control. All three fixes are specific to Linux's GNU C library; macOS uses a different allocator and is unaffected, and for now unprotected.

- **Serving a tile copied all of it to deliver 4%** ([#237](https://github.com/samsoir/xearthlayer/issues/237)): X-Plane reads about 4% of each generated texture, in two calls. Delivering it copied the whole 11.17 MiB, out of the memory cache, again through each of two request coalescers, again on the job result, and once more per reply, even when the bytes were already resident. The payload is now reference-counted from the cache to the kernel, so a read borrows the tile and slices it. Measured over an 11.5-hour flight, the read path obtained exactly the bytes it delivered at every one of 690 samples, against 25.6 times as many before.

- **Handle budget exhaustion was invisible** ([#236](https://github.com/samsoir/xearthlayer/issues/236)): Past its ceiling, the per-open tile cache silently stops memoising and every read resolves its tile again, the cost the previous release removed, reintroduced with no symptom but a rising counter that looks identical to the fix not working. Exhaustion is now counted on the `Memory sample` line and warned once at default log level, and the ceiling is logged at mount so a trace is self-describing.

- **Arch and CachyOS packages failed to build** ([#222](https://github.com/samsoir/xearthlayer/issues/222)): The release workflow re-authored the PKGBUILD from scratch rather than templating the one in the repository, and that copy omitted the flag disabling link-time optimisation. Arch enables LTO by default, which causes a dependency's C sources to compile without machine code, so linking failed with every symbol undefined. The duplicate is deleted, `.SRCINFO` is now derived rather than hand-written, and the release job builds the package with a stock configuration so a PKGBUILD that cannot build cannot ship. Reported by [@goon818](https://github.com/goon818).

- **Counters were documented as per-interval but were cumulative** ([#229](https://github.com/samsoir/xearthlayer/issues/229)): `AggregatedState::reset()` had no production caller, so every field it zeroed had always accumulated since process start, while the code, its tests and several comments read as though a reset ran, which produced at least one incorrect conclusion during earlier work. The function is removed rather than wired up: it also zeroed eight in-flight gauges maintained by increment/decrement pairs, so calling it would have stranded pending decrements and under-reported live work every interval. The documentation now states that counters are cumulative and that a rate comes from differencing successive samples.

- **Slow regions were permanently retired as having no scenery coverage** ([#226](https://github.com/samsoir/xearthlayer/issues/226)): Two observations that mean opposite things, "the scenery index attributes no tiles to this region" and "this region's tiles have not arrived yet", shared one retirement path, so a region that was merely slow was written off after three attempts and never retried. The state is now reachable only when the index genuinely attributes zero tiles. A region that stalls with tiles outstanding is instead *deferred* on a 20/30/40/60 second ladder, so its tiles stay retryable, and an on-demand request from the sim clears the deferral immediately. Reported by [@mmaechtel](https://github.com/mmaechtel).

- **Every FUSE read rebuilt the whole file or texture to answer it** ([#233](https://github.com/samsoir/xearthlayer/issues/233), [#234](https://github.com/samsoir/xearthlayer/issues/234)): The kernel caps each FUSE read at 1 MiB regardless of what the application asked for, so one texture or terrain file arrives as many ranged calls. Both read paths answered each by materialising the entire object and discarding all but the requested window, up to 238 times over for the largest installed ortho DSF, and for generated textures a full executor round trip per call, each copying the whole tile out of the memory cache twice. Passthrough files now read only the requested range, and a generated tile is produced once per file open and sliced thereafter. Measured across a four-minute scene load: 24 GiB of copying avoided.

- **An internal event queue grew without bound and was never read** ([#234](https://github.com/samsoir/xearthlayer/issues/234)): The FUSE layer sent a DDS access event on every texture read into a channel whose consumer was never connected, so the queue accumulated for the life of the session. The Scene Tracker already carries the same signal and is drained.
...
- **A damaged cache file stopped XEarthLayer from starting** ([#253](https://github.com/samsoir/xearthlayer/issues/253)): The index caches were read with no bound on how large a single field could claim to be, so one corrupt byte could be read as a length of several exabytes. The allocation failed and the process aborted mid-startup, with no error a user could act on and no remedy except deleting a file most people do not know exists. The intent to discard a bad cache and rebuild it was already in the code and could never run: an allocation failure aborts the process rather than returning an error the caller can catch. Every cache now bounds what it reads by the size of the file it is reading, writes durably so a partial file is never promoted, and deletes an entry it cannot trust so the next read regenerates it. This also repairs a second case with the same shape,  DDS tiles written by 0.4.6 are the wrong size for 0.4.7 and were served as magenta placeholders indefinitely because nothing removed them. They are now replaced on first use, so expect one slower flight after upgrading from 0.4.6 while the cache refills.

- **Regions were retired as uncovered on the strength of no evidence** ([#228](https://github.com/samsoir/xearthlayer/issues/228)): Prefetch marks a region as having no scenery so that it stops requesting tiles there. When no scenery index was loaded, the lookup returned "no tiles",  indistinguishable from an index that had been consulted and genuinely found none,  so regions were retired without anything having been checked. Anyone running without installed ortho packages had the entire world marked as uncovered. Not being able to tell is now distinguished from having looked and found nothing, and only the latter retires a region.

- **Three settings were reported by `config list` but never reached the runtime** ([#248](https://github.com/samsoir/xearthlayer/issues/248)): Each was parsed, validated and echoed back, while a hardcoded constant or a default silently won. `generation.timeout` now bounds a blocking FUSE read, which lowers the effective limit from 30 seconds to the documented default of 10 before a placeholder texture is returned. `cache.dds_disk_ratio` now sizes the DDS and chunk disk budgets, which had always split 60/40 regardless of the setting while the dashboard displayed the configured figure. `executor.max_concurrent_jobs` moved section in an earlier release without the parser following, so a configuration file XEarthLayer had written could not be read back. Reported by [@mmaechtel](https://github.com/mmaechtel).

- **`xearthlayer diagnostics` could hang indefinitely** ([#251](https://github.com/samsoir/xearthlayer/issues/251)): Measuring the cache directory walked the entire tree, which on a multi-terabyte cache did not finish. Since the bug report template asks for diagnostics output, this blocked filing a report at all. The measurement is now bounded at five seconds and reports the size as unmeasured if it does not complete, rather than the report never printing. Reported by [@mmaechtel](https://github.com/mmaechtel).
...
- **`cache.disk_io_profile`** ([#227](https://github.com/samsoir/xearthlayer/issues/227)): The setting selected an HDD, SSD or NVMe I/O profile, and nothing consumed the result,  the disk I/O limit it was supposed to size had been a fixed value for some time. It is removed rather than wired up, because the pool it described is now derived alongside the others. Storage *detection* is unaffected and still appears in `xearthlayer diagnostics`. A configuration containing the key loads normally, and `xearthlayer config upgrade` removes it.

- **Three executor resource-pool settings that never had any effect** ([#249](https://github.com/samsoir/xearthlayer/issues/249)): `executor.network_concurrent`, `executor.cpu_concurrent` and `executor.disk_io_concurrent` were parsed, validated and reported by `config list`, but no value ever reached a resource pool, the pools have always been sized from the host's logical core count by a separate, flight-tuned policy. `config list` compounded this by printing numbers from a *second* formula that contradicted the live one: on a 32-thread machine it reported a CPU capacity of 16 while the executor ran 40. Because no value on disk could describe anything the software did, none can represent a considered choice, so the keys are removed rather than wired up. Wiring them would have been a regression: a configuration written before v0.3.1 still carries the concurrency figure that release removed to fix priority inversion, and pool sizing is load-bearing for the memory behaviour fixed in [#227](https://github.com/samsoir/xearthlayer/issues/227). Pool capacities are no longer configurable. A configuration still containing the keys loads normally, and `xearthlayer config upgrade` removes them.

- **Two unreachable FUSE filesystem implementations** ([#233](https://github.com/samsoir/xearthlayer/issues/233)): `Fuse3PassthroughFS` and `Fuse3UnionFS` were both superseded by the consolidated ortho mount and neither was reachable from the running application. Removing them also retired the logger dependency-injection seam they were the last consumers of; `XEarthLayerService::start()` and `ServiceBuilder` no longer take a logger argument, which affects embedders but changes nothing about what is written to the log.
