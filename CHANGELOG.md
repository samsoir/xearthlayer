...
- **Platform-native directory layout** ([#193](https://github.com/samsoir/xearthlayer/issues/193)): XEarthLayer no longer keeps everything in `~/.xearthlayer`. Configuration, caches, data and logs now go where the system expects them: the XDG base directories on Linux, honouring `$XDG_CONFIG_HOME` and its siblings, and `Library/Application Support`, `Caches` and `Logs` on macOS. Separating the roles means a cache cleaner can reclaim regenerable tiles without touching settings, and a backup of `~/.config` captures configuration without sweeping up hundreds of gigabytes. Every location remains overridable, and one resolver now answers where anything lives rather than twenty call sites each constructing their own answer.

  Upgrading needs no action. The migration runs on first start and **relocates only `config.ini`, copying rather than moving it**. Nothing else is moved: whether two directories share a filesystem is not reliably answerable, since `rename` fails across btrfs subvolumes, bind mounts and overlayfs layers inside what `df` calls one filesystem, and a migration that moved half a terabyte on some machines and not others would be worse than one that never moves anything. Instead, anything sitting at an old location with data in it has that path written into the migrated configuration, so accepting the proposed layout cannot break a working installation. Locations with a real choice are read back for confirmation, keeping them is always the default, and without a terminal the question is deferred rather than blocking. The old configuration file is kept and rewritten so an earlier version still works if you roll back, and `xearthlayer migrate` re-runs the review, with `--dry-run` and `status`. See [directory-layout.md](docs/directory-layout.md), which also covers what is safe to delete afterwards.
...
- **`xearthlayer cache migrate`** ([#268](https://github.com/samsoir/xearthlayer/issues/268)): The command that moved a flat disk cache into region subdirectories, deprecated since the region layout became the default in v0.3.1 and marked for removal in v0.4.0. Nothing changes in practice: the cache scan only descends into region directories, so files still sitting in a flat layout have been invisible to XEarthLayer since that release whether or not this command existed. If you have not run XEarthLayer since before v0.3.1, those files are stale and can be deleted, or cleared with `xearthlayer cache clear`. The cache refills on demand.
