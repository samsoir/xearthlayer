...
- **Memory growth from tiles never returned to the operating system** ([#227](https://github.com/samsoir/xearthlayer/issues/227)): A generated tile is 11.17 MiB. glibc serves an allocation that size either by `mmap`, which returns the memory on free, or from an arena, which does not — and its threshold for choosing starts at 128 KiB and adapts *upward* every time an mmapped block is freed. It passes 11 MiB after a single tile, after which every arena that serves a tile keeps that space for the life of the process, and glibc permits up to eight arenas per core. The threshold is now pinned, at a measured cost of 0.16 ms per tile. Across a 4.3-hour flight against a same-build flight on default settings, committed memory at matched work fell 35% — 8,970 MB against 13,721 MB at 39,157 tiles. This bounds the steep early retention; it does not establish that growth is bounded over a long haul, and #227 remains open for the tail.
