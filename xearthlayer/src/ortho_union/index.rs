...
impl OrthoUnionIndex {
    ...
    ///
    /// Includes both provider-specific prefixes (BI, GO2, GO) generated by XEL
    /// and the generic "ZL" prefix used by Ortho4XP and third-party patches.
    const DDS_MAP_TYPES: &'static [&'static str] = &["ZL", "BI", "GO2", "GO"];

    /// Checks if a DDS tile exists on disk in any indexed ortho source.
    ///
    /// Used by prefetch to skip tiles that are already installed locally.
    /// This checks all known filename patterns since we want to skip prefetching
    /// regardless of the tile's origin:
    ///
    /// - `{row}_{col}_ZL{zoom}.dds` - Generic/Ortho4XP/third-party format
    /// - `{row}_{col}_BI{zoom}.dds` - Bing Maps (XEL-generated)
    /// - `{row}_{col}_GO2{zoom}.dds` - Google GO2 (XEL-generated)
    /// - `{row}_{col}_GO{zoom}.dds` - Google legacy (XEL-generated)
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// `true` if a DDS file for this tile exists in any source (with any provider prefix),
    /// `false` otherwise.
    ///
    /// # Example
    ///
    /// ```ignore
    /// if index.dds_tile_exists(100000, 125184, 18) {
    ///     // Skip prefetching - tile already exists on disk
    /// }
    /// ```
    pub fn dds_tile_exists(&self, row: u32, col: u32, zoom: u8) -> bool {
        // Check all known filename patterns (ZL generic, BI, GO2, GO provider-specific)
        // since we don't care about the tile's origin - we just want to know if it exists
        for &prefix in Self::DDS_MAP_TYPES {
            let filename = format!("{}_{}_{}{}.dds", row, col, prefix, zoom);
            let virtual_path = Path::new("textures").join(&filename);

            if self.resolve_lazy(&virtual_path).is_some() {
                return true;
            }
        }
        false
...
mod tests {
    ...
    // ========================================================================
    // dds_tile_exists tests (Issue #39)
    // ========================================================================

    #[test]
    fn test_dds_tile_exists_returns_true_for_existing_tile() {
        let temp = TempDir::new().unwrap();

        // Create a package with a DDS file in textures directory
        // Filename format: {row}_{col}_{provider}{zoom}.dds
        let source = create_package_with_dds(&temp, "test", &["100000_125184_BI18.dds"]);

        let index = OrthoUnionIndex::with_sources(vec![source]);

        // This tile exists on disk
        assert!(
            index.dds_tile_exists(100000, 125184, 18),
            "Should find existing DDS tile"
        );
    }

    #[test]
    fn test_dds_tile_exists_returns_false_for_missing_tile() {
        let temp = TempDir::new().unwrap();

        // Create a package with a different DDS file
        let source = create_package_with_dds(&temp, "test", &["100000_125184_BI18.dds"]);

        let index = OrthoUnionIndex::with_sources(vec![source]);

        // This tile does NOT exist on disk
        assert!(
            !index.dds_tile_exists(99999, 99999, 18),
            "Should not find missing DDS tile"
        );
    }

    #[test]
    fn test_dds_tile_exists_checks_multiple_providers() {
        let temp = TempDir::new().unwrap();

        // Create a package with a GO2 tile (not BI)
        let source = create_package_with_dds(&temp, "test", &["100000_125184_GO218.dds"]);

        let index = OrthoUnionIndex::with_sources(vec![source]);

        // Should find the tile regardless of which provider created it
        assert!(
            index.dds_tile_exists(100000, 125184, 18),
            "Should find DDS tile from any provider"
        );
    }

    #[test]
    fn test_dds_tile_exists_finds_zl_format_tiles() {
        let temp = TempDir::new().unwrap();

        // Create a package with a ZL-format tile (generic Ortho4XP/third-party format)
        // This is the most common format for user-installed ortho tiles
        let source = create_package_with_dds(&temp, "test", &["100000_125184_ZL18.dds"]);

        let index = OrthoUnionIndex::with_sources(vec![source]);

        // Should find tiles with the generic ZL prefix (Ortho4XP format)
        assert!(
            index.dds_tile_exists(100000, 125184, 18),
            "Should find DDS tile with generic ZL prefix (Ortho4XP format)"
        );
    }

    #[test]
    fn test_dds_tile_exists_empty_index() {
        let index = OrthoUnionIndex::new();

        // Empty index should return false
        assert!(
            !index.dds_tile_exists(100000, 125184, 18),
            "Empty index should not find any tiles"
        );
    }

    #[test]
    fn test_dds_tile_exists_checks_multiple_sources() {
        let temp = TempDir::new().unwrap();

        // Create two packages with different tiles
        let source1 = create_package_with_dds(&temp, "na", &["100000_125184_BI18.dds"]);
        let source2 = create_package_with_dds(&temp, "eu", &["200000_125184_BI18.dds"]);

        let index = OrthoUnionIndex::with_sources(vec![source1, source2]);

        // Should find tiles from both sources
        assert!(
            index.dds_tile_exists(100000, 125184, 18),
            "Should find tile from first source"
        );
        assert!(
